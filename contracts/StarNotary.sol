...
contract StarNotary is ERC721 {
    ...
    function createStar(
        ...
        require(
            starFingerprints[starFingerprint] == false,
            "Star already exists!"
        );
    ...
    function getStarFingerprint(
        ...
        uint256 starFingerprint = uint256(
            keccak256(abi.encodePacked(encodedCent, encodedDec, encodedMag))
        );
        return starFingerprint;
    }

    function concatenate(string prefix, string suffix)
        private
        pure
        returns (bytes)
    {
    ...
    function tokenIdToStarInfo(uint256 _token)
        public
        view
        returns (
            string,
